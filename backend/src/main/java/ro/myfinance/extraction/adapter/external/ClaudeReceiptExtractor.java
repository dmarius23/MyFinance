...
/**
 * Shared Claude vision receipt/invoice extractor: builds the image+prompt request and parses the strict
 * JSON response. The transport (Anthropic API vs AWS Bedrock) is the only difference, supplied by
 * {@link #invoke(List)}. Never throws — any error yields {@link ParsedReceipt#empty()} so the document
 * degrades to NEEDS_REVIEW.
 */
abstract class ClaudeReceiptExtractor implements ReceiptExtractor {

    private static final Logger log = LoggerFactory.getLogger(ClaudeReceiptExtractor.class);
    ...
    private static final String PROMPT = """
            ...
            - currency: e.g. "RON"
            - issueDate (yyyy-MM-dd): the date the document was ISSUED. On a bon fiscal it is the \
            transaction date on the "DATA:" line near the bottom, next to a time ("ORA:" / e.g. \
    ...
    /** Provider transport: send the Anthropic user-message {@code content} to Claude and return its text output. */
    protected abstract String invoke(List<Object> content);
