...
class TaxDeclarationListenerTest {
    ...
    @Test
    void inPeriodDeclarationIsKeptAndNeverFlaggedDuplicate() throws IOException {
        byte[] d100 = fixture("D100.pdf");
        assumeTrue(d100 != null, "fixtures missing");
        // Filed under its own month (March): stored, in-period, and never flagged duplicate (no auto-dedup).
    ...
    @Test
    void aMisfiledCopyIsOutsidePeriodButNotDuplicate() throws IOException {
        byte[] d100 = fixture("D100.pdf");
        assumeTrue(d100 != null, "fixtures missing");
        // The D100 is for March but filed under June. Wrong-month is still detected (outside period), but
        // with no auto-dedup it is NOT flagged duplicate — a company may legitimately have several copies.
        TaxDeclaration saved = onUpload(d100, "D100.pdf", LocalDate.of(2026, 6, 1), "49443957");
        assertThat(saved.isOutsidePeriod()).isTrue();
        assertThat(saved.isDuplicate()).isFalse();
    }
