...
/** Per-company report-email log (B skin): send history + resend + send-new. */
export function ReportLogModal({ companyId, companyName, period, onClose, onCompose }:
  ...
  const { data = [], isLoading } = useQuery({
    ...
    queryFn: () => reportsApi.history(companyId, period),
  });
  const resend = useMutation({
    mutationFn: (e: ReportEmailView) => reportsApi.send(companyId, { period, recipient: e.recipient ?? "", body: e.body }),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["report-emails", companyId, period] });
  ...
  return (
    <div style={overlay} onClick={onClose}>
      <div style={modal} onClick={(e) => e.stopPropagation()}>
        <div style={header}>
          <div>
            <div style={{ color: "var(--chrome-muted)", fontSize: 11 }}>{t("reports.emailsSent")}</div>
        ...
        <div style={{ padding: 16, overflowY: "auto" }}>
          ...
          <div style={lbl}>{t("taxes.history")}</div>
          {isLoading && <p>{t("common.loading")}</p>}
          {!isLoading && data.length === 0 && <div style={{ color: "var(--text-muted)", fontSize: 12.5, padding: "10px 0" }}>{t("reports.noEmails")}</div>}
          {data.map((e) => (
            <div key={e.id} style={{ display: "flex", gap: 10, padding: "11px 0", borderTop: "1px solid var(--hair)" }}>
              <div style={iconBox}><Icon name="reports" size={14} style={{ color: "var(--teal-chip-fg)" }} /></div>
...
const lbl: React.CSSProperties = { fontSize: 10, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--text-muted)", fontWeight: 700, marginBottom: 6 };
const overlay: React.CSSProperties = { position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "5vh 16px", zIndex: 60 };
...
const iconBox: React.CSSProperties = { width: 30, height: 30, flex: "none", borderRadius: 8, background: "var(--teal-chip-bg)", display: "grid", placeItems: "center" };
