...
/** Report email preview — one card per company; the branded PDF + charts image are attached automatically. */
export function ReportEmailModal({ targets, period, onClose }:
  ...
  useEffect(() => {
    let cancelled = false;
    targets.forEach((x) => {
      Promise.all([reportsApi.emailBody(x.companyId, period), emailApi.envelope(x.companyId)])
        .then(([b, env]) => { if (!cancelled) setDrafts((d) => ({ ...d, [x.companyId]: { ...d[x.companyId], body: b.body, recipient: env.recipient ?? "", loading: false } })); })
  ...
  const sendOne = useMutation({
    mutationFn: (v: { companyId: string; recipient: string; body: string }) =>
      reportsApi.send(v.companyId, { period, recipient: v.recipient, body: v.body }),
  });
  ...
  return (
    <div style={overlay} onClick={onClose}>
      <div style={modal} onClick={(e) => e.stopPropagation()}>
        <div style={header}>
          <div>
            <div style={{ color: "var(--chrome-muted)", fontSize: 11 }}>{t("reports.reviewBeforeSend")}</div>
            <div style={{ color: "#f3f8f7", fontSize: 17, fontWeight: 700 }}>{t("reports.emailPreviewN", { n: targets.length })}</div>
            <div style={{ color: "var(--chrome-text)", fontSize: 12 }}>{t("reports.attachmentsNote")}</div>
          </div>
              ...
              <div key={x.companyId} style={card}>
                <div style={cardHead}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={iconBox}><Icon name="reports" size={13} style={{ color: "var(--teal-chip-fg)" }} /></div>
                  ...
                  <span className="pill info round"><Icon name="doc" size={10} style={{ verticalAlign: "-1px", marginRight: 3 }} />PDF + PNG</span>
                </div>
                  ...
                  {d && !d.loading && (
                    <>
                      ...
                      <textarea value={d.body} disabled={d.sent}
                        ...
                        style={{ ...input, minHeight: 180, marginTop: 8, fontFamily: "inherit", resize: "vertical" }} />
                    </>
                  )}
                </div>
              </div>
...
const overlay: React.CSSProperties = { position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "4vh 16px", zIndex: 65 };
...
const iconBox: React.CSSProperties = { width: 26, height: 26, borderRadius: 7, background: "var(--teal-chip-bg)", display: "grid", placeItems: "center" };
