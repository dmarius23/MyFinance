...
import { remindersApi, type ReminderView } from "../api/documents";
...
/** Per-company reminder log (B skin): timeline of sends + a "send a new" bar. Triggered by the Last-sent pill. */
export function ReminderLogModal({ companyId, companyName, period, onClose, onCompose }:
  { companyId: string; companyName: string; period: string; onClose: () => void; onCompose: () => void }) {
  const { t } = useTranslation();
  const qc = useQueryClient();
  const { data = [], isLoading } = useQuery({
    queryKey: ["doc-reminder-history", companyId, period],
    queryFn: () => remindersApi.history(companyId, period),
  });

  const resend = useMutation({
    mutationFn: (e: ReminderView) => remindersApi.send(companyId, {
      period, recipient: e.recipient ?? "", body: e.body,
    }),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["doc-reminder-history", companyId, period] });
  ...
  return (
    <div style={overlay} onClick={onClose}>
      <div style={modal} onClick={(e) => e.stopPropagation()}>
        <div style={header}>
          <div>
            <div style={{ color: "var(--chrome-muted)", fontSize: 11 }}>{t("reminders.sent")}</div>
        ...
        <div style={{ padding: 16, overflowY: "auto" }}>
          ...
          {!isLoading && data.length === 0 && <div style={{ color: "var(--text-muted)", fontSize: 12.5, padding: "10px 0" }}>{t("reminders.none")}</div>}
          {data.map((e) => (
            <div key={e.id} style={{ display: "flex", gap: 10, padding: "11px 0", borderTop: "1px solid var(--hair)" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12.5, fontWeight: 600 }}>{t("reminders.reminder")}</div>
