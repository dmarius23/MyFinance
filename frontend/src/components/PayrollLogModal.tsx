...
import { payrollApi, type PayrollEmailView } from "../api/payroll";
...
/** Per-company payroll email log (B skin): timeline of sends + a "send a new" bar. */
export function PayrollLogModal({ companyId, companyName, period, onClose, onCompose }:
  { companyId: string; companyName: string; period: string; onClose: () => void; onCompose: () => void }) {
  const { t } = useTranslation();
  const qc = useQueryClient();
  const { data = [], isLoading } = useQuery({
    queryKey: ["payroll-history", companyId, period],
    queryFn: () => payrollApi.history(companyId, period),
  });

  const resend = useMutation({
    mutationFn: (e: PayrollEmailView) => payrollApi.send(companyId, {
      period, recipient: e.recipient ?? "", body: e.body,
    }),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["payroll-history", companyId, period] });
  ...
  return (
    <div style={overlay} onClick={onClose}>
      <div style={modal} onClick={(e) => e.stopPropagation()}>
        <div style={header}>
          <div>
            <div style={{ color: "var(--chrome-muted)", fontSize: 11 }}>{t("payroll.emailsSent")}</div>
        ...
        <div style={{ padding: 16, overflowY: "auto" }}>
          ...
          {!isLoading && data.length === 0 && <div style={{ color: "var(--text-muted)", fontSize: 12.5, padding: "10px 0" }}>{t("payroll.noEmails")}</div>}
          {data.map((e) => (
            <div key={e.id} style={{ display: "flex", gap: 10, padding: "11px 0", borderTop: "1px solid var(--hair)" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12.5, fontWeight: 600 }}>{e.documentIds.length} {t("payroll.attachments").toLowerCase()}</div>
